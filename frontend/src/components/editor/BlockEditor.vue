<template>
  <div class="relative">
      ...
      <div class="mb-8 group">
        ...
        <div class="flex items-center justify-between gap-3 mb-4">
          <div class="flex items-center gap-3">
            <button
              class="text-4xl hover:scale-110 transition-transform p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800"
            ...
            <button
              v-if="!pageCoverUrl"
              class="opacity-0 group-hover:opacity-100 text-xs font-medium text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 px-2.5 py-1 rounded-lg transition-all flex items-center gap-1.5"
              @click="showCoverModal = true"
            >
          ...
          <div class="flex items-center gap-2">
            ...
            <button
              class="px-2.5 py-1 rounded-lg text-xs font-medium text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex items-center gap-1.5"
              title="Insert Image"
            ...
            <button
              class="px-2.5 py-1 rounded-lg text-xs font-semibold text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/40 hover:bg-purple-100 dark:hover:bg-purple-900/60 transition-colors flex items-center gap-1.5 shadow-sm"
            ...
            <div class="relative">
              <button
                class="px-2.5 py-1 rounded-lg text-xs font-medium text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex items-center gap-1"
              ...
              <div
                v-if="showFontMenu"
                class="absolute right-0 top-full mt-1 w-36 rounded-xl bg-white dark:bg-[#202020] border border-slate-200 dark:border-[#2f2f2f] shadow-xl p-1 z-50 space-y-0.5 text-xs select-none"
              >
                <button
                  class="w-full px-2.5 py-1.5 rounded-lg text-left font-sans hover:bg-slate-100 dark:hover:bg-[#2a2a2a] flex items-center justify-between"
                  @click="fontStyle = 'sans'; showFontMenu = false"
                >
                  <span>Default (Sans)</span>
                  <Check v-if="fontStyle === 'sans'" class="w-3 h-3" />
                </button>
                <button
                  class="w-full px-2.5 py-1.5 rounded-lg text-left font-serif hover:bg-slate-100 dark:hover:bg-[#2a2a2a] flex items-center justify-between"
                  @click="fontStyle = 'serif'; showFontMenu = false"
                >
                  <span>Serif</span>
                  <Check v-if="fontStyle === 'serif'" class="w-3 h-3" />
                </button>
                <button
                  class="w-full px-2.5 py-1.5 rounded-lg text-left font-mono hover:bg-slate-100 dark:hover:bg-[#2a2a2a] flex items-center justify-between"
                  @click="fontStyle = 'mono'; showFontMenu = false"
                >
                  <span>Mono</span>
                  <Check v-if="fontStyle === 'mono'" class="w-3 h-3" />
                </button>
                <div class="border-t border-slate-100 dark:border-[#2f2f2f] my-1"></div>
                <button
                  class="w-full px-2.5 py-1.5 rounded-lg text-left hover:bg-slate-100 dark:hover:bg-[#2a2a2a] flex items-center justify-between text-[11px]"
                  @click="isSmallText = !isSmallText"
                >
                  <span>Small text</span>
                  <Check v-if="isSmallText" class="w-3 h-3 text-brand-500" />
                </button>
              </div>
            </div>
            ...
            >
              <Maximize2 v-if="!isFullWidth" class="w-3.5 h-3.5" />
        ...
        <input
          ...
          class="w-full text-4xl font-bold bg-transparent border-none outline-none text-slate-900 dark:text-slate-100 placeholder-slate-300 dark:placeholder-slate-700 tracking-tight"
          @blur="savePageTitle"
          ...
          >
            ...
            <div class="w-full min-w-0">
              ...
              <div
                ...
                class="flex items-start gap-3 p-3.5 my-2 rounded-xl bg-brand-50/70 dark:bg-brand-950/30 border border-brand-200 dark:border-brand-900/60 text-sm text-slate-800 dark:text-slate-200 shadow-sm"
              >
                <span class="text-lg select-none">{{ block.content.icon || '💡' }}</span>
                <EditableBlock
                  :id="`block-${block.id}`"
                  type="callout"
                  :model-value="block.content.text || ''"
    ...
    >
      <div class="w-full max-w-md p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl animate-scale-in">
        ...
        <div class="grid grid-cols-3 gap-2.5 mb-4">
          <button
            ...
            :key="idx"
            class="h-20 rounded-xl overflow-hidden border-2 transition-all hover:scale-105"
            :class="pageCoverUrl === img ? 'border-brand-500 ring-2 ring-brand-500/30' : 'border-transparent'"
            @click="setCover(img)"
          >
        ...
        <div class="flex items-center gap-2 mb-3">
          <input
            ...
            class="flex-1 px-3 py-2 text-xs rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 outline-none text-slate-900 dark:text-slate-100"
            @keydown.enter="setCover(customCoverInput)"
          />
          <button
            class="px-3.5 py-2 rounded-xl text-xs font-semibold text-white bg-brand-600 hover:bg-brand-500"
        ...
        <label class="w-full py-2 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 hover:border-brand-500 text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-brand-500 flex items-center justify-center gap-2 cursor-pointer transition-colors">
